...
export class AmbientiService {
  ...
  addAmbiente(ambiente: AmbienteInputDTO): Observable<AmbienteInputDTO> {
    ...
    // return of(ambiente);
    return this.http.post<AmbienteInputDTO>(this.baseUrl, ambiente, this.httpOptions)
      .pipe(
        tap((addedAmbiente: AmbienteInputDTO) => console.log('Ambiente aggiunto:', addedAmbiente)),
        catchError(this.handleError)
      );
  }

  //PUT
  updateAmbiente(id: number, ambiente: AmbienteInputDTO): Observable<AmbienteInputDTO> {
    const url = `${this.baseUrl}/${id}`;
    return this.http.put<AmbienteInputDTO>(url, ambiente, this.httpOptions)
      .pipe(
        tap((updatedAmbiente: AmbienteInputDTO) => console.log('Ambiente aggiornato:', updatedAmbiente)),
        catchError(this.handleError)
      );
  }
